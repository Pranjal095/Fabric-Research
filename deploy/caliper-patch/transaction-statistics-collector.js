...
class TransactionStatisticsCollector {
        ...
        this.stats = {
            ...
            latency: {
                successful: {
                    ...
                    max: 0,
                    total: 0,
                    all: [] // Track all latencies for percentiles
                },
                failed: {
                    min: Number.MAX_SAFE_INTEGER, // best default value for min
                    max: 0,
    ...
    _updateStatistics(txResult) {
        ...
        if (txResult.IsCommitted()) {
            ...
            this.stats.latency.successful.all.push(latency);
    ...
    /**
     * Get the P95 latency for successful TXs.
     * @return {number} The P95 latency for successful TXs.
     */
    getP95LatencyForSuccessful() {
        let latencies = this.stats.latency.successful.all || [];
        if (latencies.length === 0) return 0;
        latencies.sort((a, b) => a - b);
        let idx = Math.ceil(latencies.length * 0.95) - 1;
        return latencies[idx < 0 ? 0 : idx];
    }

    /**
     * Get the P99 latency for successful TXs.
     * @return {number} The P99 latency for successful TXs.
     */
    getP99LatencyForSuccessful() {
        let latencies = this.stats.latency.successful.all || [];
        if (latencies.length === 0) return 0;
        latencies.sort((a, b) => a - b);
        let idx = Math.ceil(latencies.length * 0.99) - 1;
        return latencies[idx < 0 ? 0 : idx];
    }
        ...
        let mergedStats = {
            ...
            latency: {
                successful: {
                    ...
                    all: currentStats.map(stat => stat.latency.successful.all || []).reduce((acc, val) => acc.concat(val), []),
                },
                failed: {
