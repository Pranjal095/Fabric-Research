...
class DefaultObserver extends TestObserverInterface {
    ...
    addLatency(max, min, avg, p95, p99) {
        ...
        if (this.testData.latency.max.length > this.testData.maxlen) {
            ...
            this.testData.latency.p95.shift();
            this.testData.latency.p99.shift();
    ...
    refreshData(updates) {
        if (updates.length === 0 || Object.entries(updates[0]).length === 0) {
            ...
            let deMax = -1, deMin = -1, deAvg = 0, deP95 = 0, deP99 = 0;
            ...
            if (txnCollectionMap.size > 0) {
                ...
                if (txnCollection.getTotalSuccessfulTx() > 0) {
                    ...
                    deP95 = txnCollection.getP95LatencyForSuccessful();
                    deP99 = txnCollection.getP99LatencyForSuccessful();
                }
            ...
            if (isNaN(deMax) || isNaN(deMin) || deAvg === 0) {
                this.addLatency(0, 0, 0, 0, 0);
            } else {
                this.addLatency(deMax, deMin, deAvg, deP95, deP99);
            }
        }
        ...
        Logger.info('[' + this.testName + ' Round ' + this.testRound + ' Transaction Info] - Submitted: ' + this.testData.summary.txSub +
            ...
            ' P95:' + (this.testData.latency.p95.length > 0 ? (this.testData.latency.p95[this.testData.latency.p95.length - 1] / 1000).toFixed(2) + 's' : '0.00s'));
    }
